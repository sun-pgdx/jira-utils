...
sub execute {
    ...
    if (lc($action) eq 'analyze-comments'){
        $self->_analyze_comments();
    }
    elsif (lc($action) eq 'missing-labels-report'){
        $self->_generate_missing_labels_report();
    }
    elsif (lc($action) eq 'missing-components-report'){
        $self->_generate_missing_components_report();
    }
    elsif (lc($action) eq 'sla-compliance-report'){
        $self->_generate_sla_compliance_report();
    }
    elsif (lc($action) eq 'all-reports'){
        $self->_generate_all_reports();
    }
    else {
        $self->{_logger}->logconfess("action '$action' is not supported at this time");
    }
}
...
sub _analyze_comments {
    ...
    my $lookup = $self->{_rest_util}->getIssuesCommentsLookupByProject($project);
    if (!defined($lookup)){
        $self->{_logger}->logconfess("lookup was not defined");
    }

    my $analyzer = PGDX::Jira::Comments::Analyzer::getInstance(issues_lookup => $lookup);
    if (!defined($analyzer)){
        $self->{_logger}->logconfess("Could not instantiate PGDX::Jira::Comments::Analyzer");
    }

    $analyzer->generateReport();
}

sub _generate_missing_labels_report {
    ...
    my $lookup = $self->{_rest_util}->getIssuesLabelsLookupByProject($project);
    if (!defined($lookup)){
        $self->{_logger}->logconfess("lookup was not defined");
    }

    my $analyzer = PGDX::Jira::Labels::Analyzer::getInstance(issues_lookup => $lookup);
    if (!defined($analyzer)){
        $self->{_logger}->logconfess("Could not instantiate PGDX::Jira::Comments::Analyzer");
    }
...
sub _generate_missing_components_report {
    ...
    my $lookup = $self->{_rest_util}->getIssuesComponentsLookupByProject($project);
    if (!defined($lookup)){
        $self->{_logger}->logconfess("lookup was not defined");
    }

    my $analyzer = PGDX::Jira::Components::Analyzer::getInstance(issues_lookup => $lookup);
    if (!defined($analyzer)){
        $self->{_logger}->logconfess("Could not instantiate PGDX::Jira::Comments::Analyzer");
    }
...
sub _generate_sla_compliance_report {
    ...
    my $lookup = $self->{_rest_util}->getIssuesSLALookupByProject($project);
    if (!defined($lookup)){
        $self->{_logger}->logconfess("lookup was not defined");
    }
    
    my $analyzer = PGDX::Jira::SLA::Analyzer::getInstance(issues_lookup => $lookup);
    if (!defined($analyzer)){
        $self->{_logger}->logconfess("Could not instantiate PGDX::Jira::Comments::Analyzer");
    }
